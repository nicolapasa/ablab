...
foreach($row2 as $r2){
	...
	$cf=$r2['cf'];
	$comune =  Utility::iniziali($r2['comune']);
	$idcomune=$r2['comune'];
	if($utenza_estera!='s'){
		$provincia = $db->getCampo('province', 'siglaprovincia', array('nomeprovincia'=>addslashes($r2['provincia'])));
...
foreach($row2 as $r2){
	...
	$provincia=$db->getCampo('province', 'siglaprovincia', array('id'=>$r2['provincia_pro']));
...
include('view_alle.php');

}

// ---------------------------------------------------------
ob_clean();
$nome_file='Fattura_'.$num_fatt.'-'.$anno.'.pdf';
