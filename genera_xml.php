...
$row = $db->selectAll('fatture', array('anno'=>2020));
...
foreach($row2 as $r2){
	...
	$comune =  utf8_decode($p->getCom($r2['comune']));
	$idcomune=$r2['comune'];
	$prov = utf8_decode($db->getCampo('province', 'siglaprovincia', array('id'=>$r2['provincia'])));
	$cap=$r2['cap'];
	$mod_pag=$r2['mod_pag'];
...
echo $nome_file='./XML/rigenerate/'.$anno.'/SM03473_'.$num_file.'.xml';

if( !file_exists('./XML/rigenerate/'.$anno)) mkdir('./XML/rigenerate/'.$anno, 0777);
...
$xml.='<ProgressivoInvio>1/'.$numFatt.'</ProgressivoInvio>';
...
foreach($row4 as $r4){
	...
	$nome_cat=$db->getCampo('categorie', 'nome', array('id'=>$r4['id_cat']));
$id_cat=$r4['id_cat'];
    $totale=$r4['impo']; //2 decimali e punto invece di virgola
	$num=1;//2.000000 num deve avere 6 decimali
             ...
             <NumeroLinea>'.$n_l.'</NumeroLinea>
               <CodiceArticolo>
                  <CodiceTipo>PROPRIETARIO</CodiceTipo>
                    <CodiceValore>'.$id_cat.'</CodiceValore>
				</CodiceArticolo>
                <Descrizione>'.$nome_cat.'</Descrizione>
...
$id_cat=$r4['id_cat'];
    //$totale=$r['impo']; //2 decimali e punto invece di virgola
	   $totale=round(((100*$r4['impo'])/122),2);
	$num=1;//2.000000 num deve avere 6 decimali
	$prezzo_uni=$totale; //6 decimali
       //sconto 2 decimali
