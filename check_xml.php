...
$path="C://xampp_new/htdocs/ablab_2019/XML/2020/";

//if(file_exists($dir)) echo "trovata";
                  ...
                  if (file_exists($path.$file)) {
                    ...
                    //print_r( $xml);
								//	$simpleXmlObj->{'some-node'}
								 $xml->FatturaElettronicaHeader->DatiTrasmissione->CodiceDestinatario. "<br>";
						 $xml->FatturaElettronicaHeader->CessionarioCommittente->DatiAnagrafici->IdFiscaleIVA->IdCodice. "<br>";
              $piva= $xml->FatturaElettronicaHeader->CessionarioCommittente->DatiAnagrafici->IdFiscaleIVA->IdCodice;
									//controllo se utenza ha il codice
									$sql="select * from admin where piva = '$piva' ";
									$row=	$db->sqlquery($sql);
									foreach($row as $r){
										$cod=$r['cod'];
