...
<br>
<span style="font-size:15px">AbLab</span><br>
<span style="font-size:11px">Laboratorio di analisi veterinarie</span>
</div>
';
// MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0)

$this->MultiCell(180,0, $logo, 'B', 'L', 0, 1, '', '', true, 0, true);*/
		}
		}
...
foreach($row as $r){
	...
	$num_pro=$id_referto.'-'.$anno2;
		...
		switch($id_cat){
			...
			default:
			$nomeEsame= ucwords($db->getCampo('esami_cat', 'abbr', array('id'=>$tipo)));
...
foreach($row as $r){
	...
	$nome_animale=$r['nome'];
 	...
 	foreach($row as $r){
		...
		$nome_esame1=ucwords($r['nome_esame1']);
		$nome_esame2=ucwords($r['nome_esame2']);
...
$pdf->MultiCell(65, 0, $text, 0, 'L', 0, 0, '', '', true, 0, true);
$text=' <b>Età:</b> '.$eta;
...
if($id_cat==6  or $id_cat==1 or $id_cat==2   or $id_cat==5){
	...
	$pdf->MultiCell(180, 1, $text, 0, 'L', 0, 1, '', '', true, 0, true);
		...
		$height='';
		//
	$file=	DIR_UPLOAD.$file;
	$foto ='
'.$height.'<img src="'.$file.'" height="auto" width="250px" />
...
$rowt=$db->selectAll('tabelle', array('id_ref'=>$id), ' tipo asc ');
 ...
 foreach($rowt as $rt){
	...
	switch($tipo_t){
		case 1:
		$text='<b>LEGENDA: S = SENSIBILE, R = RESISTENTE, I = INTERMEDIO</b>
		<p>
