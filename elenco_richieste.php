...
if(($livello == 'administrator')){
  ...
  if($_GET['anno']!=''){
	...
	include('search.php');
	$sql="select * from fatture_n where anno  ='$anno_core'";
  ...
  else
  {
	...
	$sql="select * from fatture_n where id_struttura='$id_loggata' and completa='s' and attivo ='v' and anno  ='$anno_core'";
 ...
 if($_GET['anno']!=''){
	 ...
	 Utility::array_push_associative($search, array('anno'=>$_GET['anno']));
}
if($_GET['struttura']!=''){
...
if ($_GET['tipo_esame']!='' ){
	$tipo_esame=$_GET['tipo_esame'];
	 $sql.=" and tipo = '$tipo_esame' ";

	Utility::array_push_associative($search, array('tipo_esame'=>$_GET['tipo_esame']));
...
$classTr='';
$data_arrivo='';
if( count($db->selectAll('referti', array('id_scheda'=>$id_scheda)))>0){

  $classTr='check';
  $data_arrivo=$db->getCampo('referti', 'dataArrivo', array('id_scheda'=>$id_scheda));
}
...
$nome_animale=$db->getCampo('animale', 'nome', array('id'=>$id_animale));
